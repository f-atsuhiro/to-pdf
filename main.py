...
class main(change_point,take_camera):
    def __init__(self) -> None:
        ...
        self.move_old = None
        self.bef_move = ""
    def trans(self,show_img : bool) -> Image:
        """
        変形処理をする
        """
        try:
    ...
    def save(self) -> None:
        ...
        """
        try:
            save_pic = self.trans(show_img=False)
    ...
    def set_obj(self) -> None:
        ...
        初期から置かれているボタン等を設置
        """
        def trans_false(show_bool : bool):
            def tmp():
                self.trans(show_bool)
            return tmp
        ...
        self.preview_button = ttk.Button(text="プレビュー",command=trans_false(True))
